...
class model(object):
    def __init__(self, nh, nc, ne, de):
        ...
        self.emb = shared(0.2 * uniform(-1.0, 1.0, (ne, de)).astype(floatX))
        self.Wx_enc = shared(0.2 * uniform(-1.0, 1.0, (de, nh)).astype(floatX))
        self.Wx_dec = shared(0.2 * uniform(-1.0, 1.0, (de, nh)).astype(floatX))
        self.Wh_enc = shared(0.2 * uniform(-1.0, 1.0, (nh, nh)).astype(floatX))
        self.Wh_dec = shared(0.2 * uniform(-1.0, 1.0, (nh, nh)).astype(floatX))

        self.h0_enc = shared(numpy.zeros(nh, dtype=floatX))
        self.h0_dec = shared(numpy.zeros(nh, dtype=floatX))

        self.W = shared(0.2 * uniform(-1.0, 1.0, (nh, nc)).astype(floatX))
        self.b = shared(numpy.zeros(nc, dtype=floatX))

        # bundle
        self.params = [self.emb, self.Wx_enc, self.Wx_dec, self.Wh_enc,
                       self.Wh_dec, self.W, self.b, self.h0_enc, self.h0_dec]
        ...
        def recurrence(x_t, h_tm1):
            h_t = T.nnet.sigmoid(
                T.dot(x_t, self.Wx_enc) + T.dot(h_tm1, self.Wh_enc))
        ...
        # We only keep the last hidden state
        h_enc = h[-1, :]
        ...
        # theano functions
        self.train = theano.function([idxs_enc, idxs_dec, y, lr], nll,
                                     updates=updates)
        ...
        h_t = T.nnet.sigmoid(
            T.dot(self.emb[idxs_dec], self.Wx_dec) + T.dot(h_tm1,
                                                           self.Wh_dec) + h_enc)
        s_t = T.nnet.softmax(T.dot(h_t, self.W) + self.b)

        self.compute_h_enc = theano.function([idxs_enc], h_enc)
        self.generate_step = theano.function(inputs=[h_tm1, h_enc, idxs_dec],
                                             outputs=[h_t, s_t])
    ...
    def generate_text(self, idxs_enc, max_len=10):
        ...
        for i in range(max_len):
            ...
            cur_dec_idx = \
                np.argwhere(numpy.random.multinomial(1, probas[0]) == 1)[0][0]
            y_pred += [cur_dec_idx]
...
def preprocess(x, y):
    ...
    sentence_dec = np.array([0] + y[:-1]).astype(
        'int32') - 1  # trick with 1-based indexing
...
def main(nsamples=10000,
    ...
    for epoch in range(nepochs):
        ...
        for i, (x, y) in enumerate(zip(X_train, y_train)):
            ...
            print "epoch %i - %.2f %% completed - nll = %.2f\r" % \
                  (epoch, (i + 1) * 100. / len(X_train), np.mean(nlls)),
            sys.stdout.flush()
        print
        ...
        if (epoch + 1) % val_freq == 0:
            for i, (x, y) in enumerate(zip(X_val, y_val)):
                ...
                try:
                    print "ground-truth\t", np.concatenate(
                        [[sentence_dec[1]], target[:-1]])
